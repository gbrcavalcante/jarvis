...
@pytest.mark.asyncio
async def test_hotword_fires_callback_on_match() -> None:
    ...
    with patch("openwakeword.Model") as MockModel:
...
@pytest.mark.asyncio
async def test_hotword_ignores_low_confidence() -> None:
    callback_called = False
...
def test_hotword_cpu_usage_below_threshold() -> None:
    """Smoke test: detector instantiates without loading GPU resources."""
    with patch("openwakeword.Model"):
        from src.audio.hotword import HotwordDetector
        detector = HotwordDetector(phrases=["hey_jarvis"], threshold=0.5)
